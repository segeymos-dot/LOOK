-- 067: stop auto-confirming email in handle_new_user
--
-- Root cause: since 004 / carried through 041–066, handle_new_user did:
--   UPDATE auth.users SET email_confirmed_at = COALESCE(email_confirmed_at, NOW())
-- That made every signup immediately "confirmed", so Supabase returned a session,
-- skipped confirmation email UX, and users entered LOOK without clicking a link.
--
-- Fix: keep profile + legal consent creation; do NOT touch email_confirmed_at.
-- Confirmation must come only from Supabase Auth email confirmation flow.
-- Existing confirmed users are untouched (we do not NULL out email_confirmed_at).
...
BEGIN
  -- Intentionally do NOT set auth.users.email_confirmed_at here.
