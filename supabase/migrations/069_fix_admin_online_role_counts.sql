-- 069: Fix admin customer/provider online counters.
-- Count by live profiles.role (BOTH → both counters). Platform admins excluded.
-- Do not rely solely on denormalized app_presence.is_customer / is_provider.
...
BEGIN
  ...
  -- Distinct non-admin presence rows (anon + users) within the online window.
  ...
  -- Customers online: authenticated presence + role customer|both. BOTH counts here.
  SELECT COUNT(DISTINCT p.user_id) INTO v_customers_online
  FROM public.app_presence p
  JOIN public.profiles pr ON pr.id = p.user_id
  WHERE p.last_heartbeat_at > NOW() - v_online_window
    AND p.user_id IS NOT NULL
    AND COALESCE(pr.is_platform_admin, false) = false
    AND pr.role IN ('customer', 'both');

  -- Providers online: authenticated presence + role provider|both. BOTH counts here.
  SELECT COUNT(DISTINCT p.user_id) INTO v_providers_online
  FROM public.app_presence p
  JOIN public.profiles pr ON pr.id = p.user_id
  WHERE p.last_heartbeat_at > NOW() - v_online_window
    AND p.user_id IS NOT NULL
