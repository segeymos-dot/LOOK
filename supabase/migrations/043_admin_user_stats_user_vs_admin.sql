-- Separate user/anonymous visit metrics from platform-admin sessions.
-- "Today" uses the same day boundary as role analytics: date_trunc('day', NOW())
-- (UTC / database timezone — matches get_admin_customer_stats / provider stats).
...
BEGIN
  ...
  FROM (
    SELECT
      ...
      COUNT(s.id)::bigint AS visits_total,
      COUNT(s.id) FILTER (WHERE s.started_at >= v_day_start)::bigint AS visits_today,
      MAX(s.last_activity_at) AS last_seen_at
    FROM public.profiles pr
    LEFT JOIN public.app_sessions s ON s.user_id = pr.id
    WHERE pr.is_platform_admin = true
